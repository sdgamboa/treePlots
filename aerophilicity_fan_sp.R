...
## aerobic ####
l <- list(
    fac_holdout_ring_color = holdout |> 
    ...
    fac_training_node_color = predicted |> 
        filter(Attribute == 'facultatively anaerobic') |> 
        filter(Evidence %in% ev1) |> # original
        select(NCBI_ID) |> 
        mutate(
    ...
    fac_predicted_holdout_ring_color <-  predicted |> 
        filter(Attribute == 'facultatively anaerobic') |> 
        filter(Evidence %in% ev2) |> # propagation 
...
df <- bind_rows(l)
fname <- 'aerophilicity_fan_sp.annot'
write.table(
